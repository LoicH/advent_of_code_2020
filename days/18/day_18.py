...
if __name__ == "__main__":
    ...
    for ex, res in examples.items():
        ...
        assert(part_two([parse_line(ex)], verbose=True) == res[1]), "Wrong output for small example {}".format(ex)

    input_ex = parse_input("input_example.txt", verbose=True)
    assert(part_one(input_ex, verbose=False) == sum([elt[0] for elt in examples.values()])), "Wrong output for part one"
